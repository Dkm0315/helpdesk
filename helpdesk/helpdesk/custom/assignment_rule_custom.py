...
def get_user_round_robin_filtered(self):
    ...
    print(f"[ASSIGNMENT DEBUG] Round Robin - Rule: {self.name}, Date: {check_date}")
    
    # Get users from self.users (directly added users)
    if self.users:
        print(f"[ASSIGNMENT DEBUG] Found {len(self.users)} direct users in rule")
        for user_obj in self.users:
            if user_obj.user:
                all_users.append(user_obj.user)
                user_to_obj_map[user_obj.user] = user_obj
                print(f"[ASSIGNMENT DEBUG] Added direct user: {user_obj.user}")
    ...
    if hasattr(self, 'custom_dynamic_user_assignment') and self.custom_dynamic_user_assignment:
        print(f"[ASSIGNMENT DEBUG] Found {len(self.custom_dynamic_user_assignment)} dynamic user assignments")
        for row in self.custom_dynamic_user_assignment:
            assignment_id = getattr(row, 'assignment_id', None)
            if assignment_id:
                print(f"[ASSIGNMENT DEBUG] Fetching users from Dynamic Assignment: {assignment_id}")
                ...
                print(f"[ASSIGNMENT DEBUG] Got {len(users)} users from Dynamic Assignment {assignment_id}")
                for user_dict in users:
                    ...
                    if user_email and user_email not in all_users:
                        ...
                        print(f"[ASSIGNMENT DEBUG] Added user from Dynamic Assignment: {user_email}")
    ...
    if not all_users and hasattr(self, 'custom_user_assignment') and self.custom_user_assignment:
        print(f"[ASSIGNMENT DEBUG] No users found, checking custom_user_assignment: {self.custom_user_assignment}")
        users = get_users_from_dynamic_assignment(self.custom_user_assignment, self.name, check_date)
        print(f"[ASSIGNMENT DEBUG] Got {len(users)} users from custom_user_assignment")
        for user_dict in users:
            user_email = user_dict.get('email') or user_dict.get('user')
            if user_email and user_email not in all_users:
                all_users.append(user_email)
                user_to_obj_map[user_email] = frappe._dict({'user': user_email})
                print(f"[ASSIGNMENT DEBUG] Added user from custom_user_assignment: {user_email}")
    
    print(f"[ASSIGNMENT DEBUG] Total users before filtering: {len(all_users)}")
    if not all_users:
        print(f"[ASSIGNMENT DEBUG] No users found for assignment rule {self.name}")
        return None
    ...
    for user in all_users:
        ...
        if not should_exclude:
            ...
            print(f"[ASSIGNMENT DEBUG] Excluded user {user} (on leave/holiday)")
    
    print(f"[ASSIGNMENT DEBUG] Available users after filtering: {len(available_users)}")
    print(f"[ASSIGNMENT DEBUG] Excluded users: {excluded_users}")
    ...
    if not available_users:
        ...
        print(f"[ASSIGNMENT DEBUG] ERROR: {error_msg}")
    ...
    if not filtered_user_objects:
        print(f"[ASSIGNMENT DEBUG] No filtered user objects found")
        return None
    ...
    if not self.last_user or self.last_user == filtered_user_objects[-1].user:
        ...
        print(f"[ASSIGNMENT DEBUG] Selected user (first/last): {selected_user}")
    ...
    for i, d in enumerate(filtered_user_objects):
        if self.last_user == d.user:
            if i + 1 < len(filtered_user_objects):
                ...
                print(f"[ASSIGNMENT DEBUG] Selected user (next): {selected_user}")
                return selected_user
            else:
                selected_user = filtered_user_objects[0].user
                print(f"[ASSIGNMENT DEBUG] Selected user (wrap around): {selected_user}")
                return selected_user
    
    # bad last user, assign to the first one
    selected_user = filtered_user_objects[0].user
    print(f"[ASSIGNMENT DEBUG] Selected user (fallback): {selected_user}")
    return selected_user
...
def get_user_load_balancing_filtered(self):
    ...
    print(f"[ASSIGNMENT DEBUG] Load Balancing - Rule: {self.name}, Date: {check_date}")
    ...
    for user in available_users:
        ...
        print(f"[ASSIGNMENT DEBUG] User {user} has {count} open assignments")
    ...
    print(f"[ASSIGNMENT DEBUG] Selected user (load balancing): {selected_user}")
    return selected_user
...
def get_user_filtered(self, doc):
    ...
    print(f"[ASSIGNMENT DEBUG] get_user_filtered - Rule: {self.name}, Rule Type: {self.rule}")
    print(f"[ASSIGNMENT DEBUG] Document: {doc.get('name')}, Category: {doc.get('custom_category')}, Status: {doc.get('status')}")
...
def safe_eval_filtered(self, fieldname, doc):
    ...
    try:
        ...
        if not condition:
            print(f"[ASSIGNMENT DEBUG] safe_eval - Rule: {self.name}, Field: {fieldname}, No condition set")
            return False
        ...
        # Debug: Print condition and doc values
        print(f"[ASSIGNMENT DEBUG] safe_eval - Rule: {self.name}, Field: {fieldname}")
        print(f"[ASSIGNMENT DEBUG] Condition: {condition}")
        print(f"[ASSIGNMENT DEBUG] Doc values - name: {doc.get('name')}, custom_category: {doc.get('custom_category')}, status: {doc.get('status')}, agent_group: {doc.get('agent_group')}")
        
        # Call frappe.safe_eval with proper dict
        result = frappe.safe_eval(condition, None, doc)
        print(f"[ASSIGNMENT DEBUG] Condition evaluation result: {result}")
        return result
        ...
        error_msg = str(e)
        print(f"[ASSIGNMENT DEBUG] ERROR in safe_eval - Rule: {self.name}, Field: {fieldname}, Error: {error_msg}")
        frappe.log_error(
...
def do_assignment_filtered(self, doc):
    ...
    print(f"[ASSIGNMENT DEBUG] do_assignment_filtered - Rule: {self.name}, DocType: {doctype}, Name: {name_str}")
    ...
    print(f"[ASSIGNMENT DEBUG] Existing assignments before clearing: {len(existing_assignments)}")
    for existing in existing_assignments:
        print(f"[ASSIGNMENT DEBUG]   - ToDo {existing.name}: {existing.allocated_to} (Rule: {existing.assignment_rule})")
    
    # Clear existing assignment (doc.get() works on Document objects)
    assign_to.clear(doctype, name_str, ignore_permissions=True)
    print(f"[ASSIGNMENT DEBUG] Cleared existing assignments")
    
    # Get user (this will use our filtered method which handles dict conversion)
    user = self.get_user(doc)
    
    if user:
        print(f"[ASSIGNMENT DEBUG] Selected user for assignment: {user}")
        ...
        if duplicate_check:
            print(f"[ASSIGNMENT DEBUG] WARNING: Duplicate assignment detected! ToDo IDs: {[d.name for d in duplicate_check]}")
        ...
        print(f"[ASSIGNMENT DEBUG] Created ToDo {todo_doc.name} for user {user}")
        ...
        try:
            ...
            if frappe.db.exists(doctype, name_str):
                if frappe.get_meta(doctype).get_field("assigned_to"):
                    ...
                    print(f"[ASSIGNMENT DEBUG] Updated assigned_to field to {user}")
        except Exception as e:
            print(f"[ASSIGNMENT DEBUG] Could not update assigned_to field: {str(e)}")
        
        # set for reference in round robin
        self.db_set("last_user", user)
        return True
    else:
        print(f"[ASSIGNMENT DEBUG] No user selected for assignment")
